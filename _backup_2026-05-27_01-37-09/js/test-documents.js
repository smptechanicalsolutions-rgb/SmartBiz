...
(function (global) {
  ...
  function openHistoryDocument(id) {
    ...
    window.location.href = "formats/" + (entry.format || "format1.html");
  }
        ...
        const recentHtml =
          recent.length === 0
            ...
            : recent
                ...
                .join("");
        return `
        <div class="dashboard-card" onclick="navigateTo('${card.navigate}')">
  ...
  function renderTransactionsList(history) {
    ...
    if (history.length === 0) {
      container.innerHTML =
        '<p style="padding:24px;text-align:center;color:#666;">No saved documents yet. Use the test documents from the User Guide or create a new invoice.</p>';
      return;
    }
