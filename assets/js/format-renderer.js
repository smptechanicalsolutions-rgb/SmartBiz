(function () {
  ...
  function calculate(data) {
    ...
    const taxable = items.reduce((sum, item) => {
      ...
      return roundTo2(sum + amount);
    }, 0);
    const shippingTotal = roundTo2(data.shipping && data.shipping.total ? Number(data.shipping.total) : 0);
    const includeCharges = docType !== "Delivery Challan";
    const taxes = typeof normalizeInvoiceTaxes === "function" ? normalizeInvoiceTaxes(data.taxes) : Array.isArray(data.taxes) ? data.taxes : [];
    let taxTotal = 0;
    const taxLines = [];

    if (includeCharges && Array.isArray(taxes)) {
      taxes.forEach((tax) => {
        const basePercent = Number(tax.percent) || 0;
        const percent = tax.type === "GST" ? basePercent * 2 : basePercent;
        const amount = roundTo2((taxable + shippingTotal) * (percent / 100));
        taxTotal = roundTo2(taxTotal + amount);
        taxLines.push({
    ...
    const grandTotal = roundTo2(includeCharges ? taxable + shippingTotal + taxTotal : taxable);
...
${escapeHtml(data.companyAddress || "")}
  ...
  function commonBody(data, totals) {
    ...
    return `
      ...
      <section class="signature-section generated-signature-section">
        <div class="signature-right generated-signature">
          <div class="sig-stamp-slot">
  ...
  function itemRow(item, index) {
    ...
    return `
      <tr>
        ...
        <td>${escapeHtml(item.tax ? item.tax + "%" : "-")}</td>
        <td class="rate">${money(rate)}</td>
        <td class="qty">${qty} ${escapeHtml(item.per || "")}</td>
  ...
  function renderActions() {
    return `
      <div class="action-buttons">
        ...
        <button class="pdf-btn" onclick="editRenderedDocument()" style="background:#334155">Edit</button>
  ...
  function editorUrl() {
    ...
    const type = String(docType || "invoice").toLowerCase().replace(/\s+/g, "-");
    return `../../index.html?format=${encodeURIComponent(formatFile)}&type=${encodeURIComponent(type)}`;
  }

  function editRenderedDocument() {
    const data = window.invoicePreviewData || readInvoiceData();
    if (data) {
      data.docType = docType;
      localStorage.setItem("invoiceData", JSON.stringify(data));
    }
    localStorage.setItem("selectedInvoiceFormat", window.location.pathname.split("/").pop() || "format1.html");
    window.location.href = editorUrl();
  }

  window.editRenderedDocument = editRenderedDocument;

  function deleteCurrentDocument() {
    if (typeof window.SBDeleteCurrentDocument === "function") {
      window.SBDeleteCurrentDocument();
      return;
    }
