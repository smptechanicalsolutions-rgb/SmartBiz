function previewInvoice() {
  ...
  const invoiceData = {
    ...
    shipping: (function(){
      let rows = Array.from(document.querySelectorAll('#shippingContainer .shipping-row'));
      if (!rows.length) {
        const descEl = document.querySelector('.shipping-desc');
        const amtEl = document.querySelector('.shipping-amount');
        if (descEl || amtEl) rows = [{ querySelector: (selector) => selector === '.shipping-desc' ? descEl : amtEl }];
      }
      const list = [];
      let total = 0;
      rows.forEach(r => {
      ...
      if (list.length === 0) {
        try {
          ...
          if (saved && (saved.desc || saved.description || saved.total || saved.amount)) {
            const desc = saved.desc || saved.description || '';
            const amt = Number(saved.total != null ? saved.total : saved.amount) || 0;
            list.push({ desc, amount: amt });
            total = amt;
          }
        } catch (e) {
          // ignore malformed saved shipping
        }
      }
    ...
    taxes: (function(){
      ...
      const list = [];
      rows.forEach(r => {
        const rawType = (r.querySelector('.tax-type')||{}).value || '';
        ...
        if (type === 'GST') {
          const totalGstRates = [5, 12, 18, 28];
          const shouldSplitTotalGst = totalGstRates.includes(percent) || percent > 12;
          const cgstPercent = shouldSplitTotalGst ? percent / 2 : percent;
          list.push({ type: 'CGST', percent: cgstPercent, desc });
          list.push({ type: 'SGST', percent: cgstPercent, desc });
        } else {
          list.push({ type: type, percent, desc });
        }
      });
      return list;
    })(),
    ...
    watermark: (function () {
      ...
      const enabled = !!(wmEnableEl && wmEnableEl.checked) || !!settings.watermarkEnable;
      const text = ((wmTextEl && wmTextEl.value) || settings.watermarkText || "").toString().trim();
  ...
  // Open preview using saved invoice data from localStorage instead of a long URL parameter.
  window.open(`documents/formats/${selectedFormat}`, "_blank");
}
...
function saveInvoiceToHistory(invoiceData) {
  try {
    ...
    if (
      ...
      selectedFormat === "format23.html" ||
      selectedFormat === "format24.html"
    ) {
      ...
      selectedFormat === "format25.html" ||
      selectedFormat === "format26.html"
    ) {
      docType = "Quotation";
    } else if (
      ...
      selectedFormat === "format27.html" ||
      selectedFormat === "format28.html"
    ) {
      docType = "Proforma";
    }
    ...
    if (invoiceData.items && invoiceData.items.length > 0) {
      invoiceData.items.forEach(item => {
        const amount = roundTo2((item.qty || 0) * (item.rate || 0));
    ...
    const grandTotal = isQuotation ? taxable : roundTo2(taxable + totalTax + shippingTotal);
...
async function saveHistoryToFile(historyEntry) {
  try {
    // If firebaseService is available and user is authenticated, try Firestore first
    if (window.firebaseService && typeof window.firebaseService.createInvoice === 'function') {
      try {
        const res = await window.firebaseService.createInvoice(historyEntry);
        return { ok: true, source: 'firestore', id: res && res.id };
      } catch (e) {
        console.warn('Firestore save failed, falling back to API:', e);
...
function saveShippingCharge() {
  try {
    const descEl = document.querySelector('#shippingContainer .shipping-desc') || document.querySelector('.shipping-desc');
    const amtEl = document.querySelector('#shippingContainer .shipping-amount') || document.querySelector('.shipping-amount');
    const desc = descEl ? descEl.value.trim() : '';
    const total = amtEl ? (Number(amtEl.value) || 0) : 0;
    const obj = { desc, description: desc, total, amount: total };
...
function loadSavedShippingCharge() {
  try {
    ...
    const descEl = document.querySelector('#shippingContainer .shipping-desc') || document.querySelector('.shipping-desc');
    const amtEl = document.querySelector('#shippingContainer .shipping-amount') || document.querySelector('.shipping-amount');
    if (descEl) descEl.value = saved.desc || saved.description || '';
    if (amtEl) amtEl.value = saved.total != null ? saved.total : (saved.amount != null ? saved.amount : '');
...
function loadInvoiceData(){
  ...
  if (companyNameEl) companyNameEl.value = data.companyName || '';
  if (companyGSTEl) companyGSTEl.value = data.companyGST || '';
  if (companyAddressEl) companyAddressEl.value = data.companyAddress || '';
  ...
  if (clientGSTEl) clientGSTEl.value = data.clientGST || '';
  if (clientAddressEl) clientAddressEl.value = data.clientAddress || '';
  ...
  if (invoiceDateEl) invoiceDateEl.value = data.invoiceDate || '';
  ...
  if(document.getElementById('dueDate')) {
    ...
    const isQuotation = selectedFormat === 'format5.html' || selectedFormat === 'format6.html' || selectedFormat === 'format12.html' || selectedFormat === 'format17.html' || selectedFormat === 'format18.html' || selectedFormat === 'format25.html' || selectedFormat === 'format26.html';
      ...
      data.items.forEach(it => {
        ...
        row.innerHTML = `
          ...
          <div class="col-6 col-md-2 mb-2 mb-md-0">
            <input class="form-control item-hsn" placeholder="HSN/SAC" value="${(it.hsn||'').replace(/"/g,'&quot;')}">
          </div>
          <div class="col-6 col-md-2 mb-2 mb-md-0">
            <input type="number" class="form-control item-tax" placeholder="Tax %" value="${it.tax||''}">
          </div>
          <div class="col-6 col-md-2 mb-2 mb-md-0">
          ...
          <div class="col-6 col-md-2 mb-2 mb-md-0">
            <input class="form-control item-per" placeholder="Per" value="${(it.per||'Nos').replace(/"/g,'&quot;')}">
          </div>
        ...
        row.innerHTML = `
          <div class="col-12 col-md-4 mb-2 mb-md-0">
            <select class="form-select tax-type">
              <option value="GST" ${['GST','CGST','SGST'].includes(t.type||'') ? 'selected' : ''}>GST (CGST + SGST)</option>
...
// Render a list of invoices into a container using firebaseService.listInvoices() or localStorage fallback
async function listInvoicesToUI(containerId = 'invoiceHistoryContainer', limit = 1000) {
  ...
  try {
    if (window.firebaseService && typeof window.firebaseService.listInvoices === 'function') {
      invoices = await window.firebaseService.listInvoices({ limit });
    } else {
      invoices = JSON.parse(localStorage.getItem('invoiceHistory') || '[]');
      // normalize structure
      invoices = invoices.map(i => ({ id: i.id, invoiceNo: i.invoiceNo, invoiceDate: i.invoiceDate, clientName: i.clientName, grandTotal: i.grandTotal, docType: i.docType, format: i.format }));
    }
  } catch (e) {
    console.error('listInvoicesToUI error', e);
    container.innerHTML = '<div class="error">Failed to load invoices.</div>';
    return;
  }

  // Prepare UI controls: search, date range, migrate button
  const controls = document.createElement('div');
  controls.className = 'invoice-controls';
  controls.style.cssText = 'display:flex;gap:8px;align-items:center;margin-bottom:10px;flex-wrap:wrap;';

  const searchInput = document.createElement('input');
  searchInput.type = 'search';
  searchInput.placeholder = 'Search invoice no / client name...';
  searchInput.style.cssText = 'padding:8px 10px;border:1px solid #ddd;border-radius:6px;min-width:220px;';
  controls.appendChild(searchInput);

  const fromDate = document.createElement('input');
  fromDate.type = 'date';
  fromDate.title = 'From date';
  fromDate.style.cssText = 'padding:6px;border:1px solid #ddd;border-radius:6px;';
  controls.appendChild(fromDate);

  const toDate = document.createElement('input');
  toDate.type = 'date';
  toDate.title = 'To date';
  toDate.style.cssText = 'padding:6px;border:1px solid #ddd;border-radius:6px;';
  controls.appendChild(toDate);

  const migrateBtn = document.createElement('button');
  migrateBtn.className = 'btn btn-sm btn-primary';
  migrateBtn.textContent = 'Migrate Local History';
  migrateBtn.style.cssText = 'padding:8px 12px;border-radius:6px;';
  controls.appendChild(migrateBtn);

  const dryRunChk = document.createElement('input');
  dryRunChk.type = 'checkbox';
  dryRunChk.id = 'migrateDryRun';
  const dryRunLbl = document.createElement('label');
  dryRunLbl.htmlFor = 'migrateDryRun';
  dryRunLbl.textContent = 'Dry run';
  dryRunLbl.style.cssText = 'margin-left:6px;font-size:0.9rem;color:#444;';
  controls.appendChild(dryRunChk);
  controls.appendChild(dryRunLbl);
  ...
  table.innerHTML = '<thead><tr><th>Invoice</th><th>Client</th><th>Type</th><th>Total</th><th>Date</th><th></th></tr></thead>';
  ...
  function renderRows(list) {
    ...
    list.forEach(inv => {
      ...
      const no = document.createElement('td'); no.textContent = inv.invoiceNo || inv.id || '';
      const client = document.createElement('td'); client.textContent = inv.clientName || '';
      const type = document.createElement('td'); type.textContent = inv.docType || '';
      const total = document.createElement('td'); total.textContent = inv.grandTotal || '';
      const date = document.createElement('td'); date.textContent = inv.invoiceDate ? new Date(inv.invoiceDate).toLocaleString() : '';
      ...
      viewBtn.onclick = function() {
        try {
          const full = inv.fullData || inv;
          localStorage.setItem('invoiceData', JSON.stringify(full));
          window.open('documents/formats/' + (inv.format || 'format1.html'), '_blank');
        } catch (e) { console.error(e); }
      };
      actions.appendChild(viewBtn);
      tr.appendChild(no); tr.appendChild(client); tr.appendChild(type); tr.appendChild(total); tr.appendChild(date); tr.appendChild(actions);
  ...
  // Filtering logic (server-side when available)
  const serverSide = !!(window.firebaseService && typeof window.firebaseService.listInvoices === 'function');
  async function applyFilters() {
    ...
    if (serverSide) {
      try {
        const opts = { limit, q: q || undefined, from: from || undefined, to: to || undefined };
        const fetched = await window.firebaseService.listInvoices(opts);
        renderRows(fetched || []);
      } catch (e) {
        console.error('server-side filter error', e);
        renderRows([]);
      }
      return;
    }
    ...
    if (qLower) {
      filtered = filtered.filter(i => ((i.invoiceNo||'') + ' ' + (i.clientName||'')).toLowerCase().includes(qLower));
    }
    if (from) {
      filtered = filtered.filter(i => i.invoiceDate && new Date(i.invoiceDate) >= from);
    }
    if (to) {
      const toEnd = new Date(to); toEnd.setHours(23,59,59,999);
      filtered = filtered.filter(i => i.invoiceDate && new Date(i.invoiceDate) <= toEnd);
    }
    renderRows(filtered);
  }
  ...
  migrateBtn.addEventListener('click', async function() {
    const dry = !!document.getElementById('migrateDryRun').checked;
    if (!confirm(dry ? 'Run migration in dry-run mode? This will not write to Firestore.' : 'Migrate local invoice history into Firestore? This will write data. Proceed?')) return;
    migrateBtn.disabled = true;
    migrateBtn.textContent = dry ? 'Dry running...' : 'Migrating...';
    try {
      if (window.firebaseService && typeof window.firebaseService.migrateLocalHistoryToFirestore === 'function') {
        const res = await window.firebaseService.migrateLocalHistoryToFirestore({ dryRun: dry });
        alert(`Migration complete. total=${res.total}, migrated=${res.migrated}, skipped=${res.skipped}, errors=${res.errors.length}`);
        // refresh listing from Firestore
        invoices = await (window.firebaseService && window.firebaseService.listInvoices ? window.firebaseService.listInvoices({ limit }) : JSON.parse(localStorage.getItem('invoiceHistory') || '[]'));
        applyFilters();
      } else {
        alert('Migration unavailable: firebaseService not loaded.');
      }
    } catch (e) {
      console.error('Migration failed', e);
      alert('Migration failed: ' + (e && e.message));
    } finally {
...
// Run migration of localStorage invoiceHistory into Firestore (if available)
async function runLocalHistoryMigration(dryRun = false) {
  if (!window.firebaseService || typeof window.firebaseService.migrateLocalHistoryToFirestore !== 'function') {
    alert('Migration not available: firebaseService not loaded');
    return;
  }
  try {
    const res = await window.firebaseService.migrateLocalHistoryToFirestore({ dryRun });
    alert(`Migration complete. total=${res.total}, migrated=${res.migrated}, skipped=${res.skipped}, errors=${res.errors.length}`);
    await listInvoicesToUI();
  } catch (e) {
...
// Auto-run listing if container exists on DOM ready
document.addEventListener('DOMContentLoaded', function() {
  if (document.getElementById('invoiceHistoryContainer')) {
    listInvoicesToUI('invoiceHistoryContainer', 1000);
  }
});
...
function loadSavedCompanyOnStart() {
  // Only load company if there's no invoice data to avoid overwriting existing draft
  const invoiceData = localStorage.getItem('invoiceData');
  if (!invoiceData) {
    // Check if there's a default company saved first
    ...
    // If no saved company exists, auto-populate from personal info
    loadCompanyFromPersonalInfo();
  }
}

function getPersonalInfoCompanyData() {
  const personalInfoJson = localStorage.getItem('personalInfo');
  if (!personalInfoJson) return null;

  let info;
  try {
    info = JSON.parse(personalInfoJson);
  } catch (e) {
    return null;
  }

  const companyName = (info.businessName || info.businessName2 || info.fullName || '').trim();
  const addressParts = [];
  if (info.address && info.address.trim()) addressParts.push(info.address.trim());
  if (info.city && info.city.trim()) addressParts.push(info.city.trim());
  if (info.state && info.state.trim()) addressParts.push(info.state.trim());
  if (info.pinCode && info.pinCode.trim()) addressParts.push(info.pinCode.trim());
  if (info.country && info.country.trim()) addressParts.push(info.country.trim());

  const companyAddress = addressParts.join(', ').trim();
  const companyGST = (info.gstin || '').trim();
  const companyMobile = (info.mobileNumber || '').trim();
  const companyEmail = (info.email || '').trim();

  if (!companyName && !companyGST && !companyAddress && !companyMobile && !companyEmail) {
    return null;
  }
...
function loadCompanyFromPersonalInfo() {
  const companyData = getPersonalInfoCompanyData();
  if (!companyData) return;

  const companyNameEl = document.getElementById('companyName');
  const companyGstEl = document.getElementById('companyGST');
  const companyAddressEl = document.getElementById('companyAddress');
  const companyMobileEl = document.getElementById('companyMobile');
  const companyEmailEl = document.getElementById('companyEmail');

  const hasAnyCurrentValue =
    (companyNameEl && companyNameEl.value.trim()) ||
    (companyGstEl && companyGstEl.value.trim()) ||
    (companyAddressEl && companyAddressEl.value.trim()) ||
    (companyMobileEl && companyMobileEl.value.trim()) ||
    (companyEmailEl && companyEmailEl.value.trim());

  if (hasAnyCurrentValue) {
    return;
  }

  if (companyNameEl && companyData.companyName) {
    companyNameEl.value = companyData.companyName;
  }
  if (companyGstEl && companyData.companyGST) {
    companyGstEl.value = companyData.companyGST;
  }
  if (companyAddressEl && companyData.companyAddress) {
    companyAddressEl.value = companyData.companyAddress;
  }
  if (companyMobileEl && companyData.companyMobile) {
    companyMobileEl.value = companyData.companyMobile;
  }
  if (companyEmailEl && companyData.companyEmail) {
    companyEmailEl.value = companyData.companyEmail;
  }

  const companySection = document.getElementById('companyFormSection');
  const companyBtn = document.getElementById('toggleCompanyBtn');
  if (companySection && companyBtn && companySection.style.display === 'none') {
    companySection.style.display = 'block';
    companyBtn.textContent = 'Hide Company';
    companyBtn.classList.remove('btn-primary');
    companyBtn.classList.add('btn-secondary');
...
function applyPersonalInfoFallbackToCompanyForm() {
  const companyNameEl = document.getElementById('companyName');
  const companyGstEl = document.getElementById('companyGST');
  const companyAddressEl = document.getElementById('companyAddress');
  const companyMobileEl = document.getElementById('companyMobile');
  ...
  const currentCompanyValueExists =
    (companyNameEl && companyNameEl.value.trim()) ||
    (companyGstEl && companyGstEl.value.trim()) ||
    (companyAddressEl && companyAddressEl.value.trim()) ||
    (companyMobileEl && companyMobileEl.value.trim()) ||
...
if(document.readyState === 'loading'){
  window.addEventListener('DOMContentLoaded', function() {
    ...
    applyPersonalInfoFallbackToCompanyForm();
    initSignaturePad();
...
function addItemRow() {
  ...
  if (!container) return;
  const row = document.createElement('div');
  row.className = 'row g-3 item-row mb-2';
        row.innerHTML = `
    <div class="col-12 col-md-4 mb-2 mb-md-0">
    ...
    <div class="col-6 col-md-2 mb-2 mb-md-0">
      <input class="form-control item-hsn" placeholder="HSN/SAC">
    </div>
    <div class="col-6 col-md-2 mb-2 mb-md-0">
      <input type="number" class="form-control item-tax" placeholder="Tax %">
    </div>
    <div class="col-6 col-md-2 mb-2 mb-md-0">
    ...
    <div class="col-6 col-md-2 mb-2 mb-md-0">
      <input class="form-control item-per" placeholder="Per" value="Nos">
    </div>
    <div class="col-2 col-md-2 d-flex align-items-center justify-content-center">
      <button type="button" class="btn btn-danger" onclick="removeItemRow(this)">-</button>
    </div>
...
async function viewHistoryItem(id) {
  ...
  // Open preview
  window.open("documents/formats/" + (historyEntry.format || "format1.html"), "_blank");
  closeHistoryModal();
}
...
function loadInvoiceDataFromHistory(data) {
    ...
    if (itemsContainer) {
      ...
      data.items.forEach(item => {
        ...
        if (lastRow) {
          ...
          const taxInput = lastRow.querySelector('.item-tax');
          const perInput = lastRow.querySelector('.item-per');
          ...
          if (perInput) perInput.value = item.per || 'Nos';
        }
      });
    }
  }
  ...
  if (data.shipping && data.shipping.items && data.shipping.items.length > 0) {
    ...
    if (shippingContainer) {
      ...
      const first = data.shipping.items[0];
      const descInput = document.querySelector('.shipping-desc');
      const amountInput = document.querySelector('.shipping-amount');
      if (descInput) descInput.value = first.desc || '';
      if (amountInput) amountInput.value = first.amount != null ? first.amount : (data.shipping.total || '');
    }
  }
...
function convertDocument(id, targetType) {
  try {
    ...
    const previewWindow = window.open("documents/formats/" + targetFormat, "_blank");
...
async function deleteDocument(id) {
  try {
    if (typeof deleteHistoryItem === 'function') {
    ...
    try {
      ...
      console.warn('Could not delete from server history, updating local history only:', apiError);
    }
...
function getCurrentProductItems() {
  ...
  rows.forEach(row => {
    ...
    const perEl = row.querySelector('.item-per');
    ...
    if (name || desc || qty || rate || hsn || tax) {
      items.push({ name, desc, qty, rate, hsn, tax, per });
    }
  });
...
function normalizeTaxTemplates(rawTemplates) {
  if (!rawTemplates) return {};
  if (Array.isArray(rawTemplates)) {
    return rawTemplates.reduce((acc, template) => {
      const name = template && template.name ? template.name.trim() : '';
      if (name) acc[name] = { ...template, name };
      return acc;
    }, {});
  }
  if (typeof rawTemplates === 'object' && rawTemplates !== null) {
    return Object.keys(rawTemplates).reduce((acc, key) => {
      const value = rawTemplates[key];
      const name = value && value.name ? String(value.name).trim() : String(key).trim();
      if (name) acc[name] = { ...value, name };
      return acc;
    }, {});
  }
  return {};
...
function saveTaxTemplate() {
  ...
  const templateName = taxes.map(t => `${t.type}-${t.percent}%`).join(', ').trim();
  
  // Get all tax templates from localStorage
  let taxTemplates = normalizeTaxTemplates(JSON.parse(localStorage.getItem('taxTemplates')) || {});
  
  // Save or overwrite current tax data by template name
...
function loadTaxTemplatesDropdown() {
  ...
  let templates = normalizeTaxTemplates(JSON.parse(localStorage.getItem('taxTemplates')) || {});
  ...
  Object.keys(templates)
    ...
    .forEach(templateName => {
      ...
      if (!normalizedName || added.has(uniqueKey)) return;
      added.add(uniqueKey);
      const option = document.createElement('option');
...
function loadSavedTaxTemplate(templateName) {
  ...
  const templates = normalizeTaxTemplates(JSON.parse(localStorage.getItem('taxTemplates')) || {});
  const template = templates[templateName.trim()];
