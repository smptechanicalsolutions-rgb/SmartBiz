/**
 * Currency display: amounts are stored in INR. When USD or EUR is selected,
 * values convert using exchange rates refreshed hourly when possible.
 */
(function () {
  const CURRENCY_MAP = {
  ...
  function normalizeCurrencyKey(value) {
    return CURRENCY_MAP[value] ? value : "INR";
  }

  function getMeta() {
    const key = normalizeCurrencyKey(getSettings().currency || "INR");
    return CURRENCY_MAP[key] || CURRENCY_MAP.INR;
  }
  ...
  async function fetchRatesFromNetwork() {
    try {
      ...
      if (res.ok) {
        ...
        if (data && data.rates) return saveRatesCache(data.rates, "server");
      }
    } catch (e) {
      /* try public API */
    }
  ...
  function convertFromInr(amountInr, targetCode) {
    ...
    const cached = getCachedRates();
    const rate = cached && cached.rates && Number(cached.rates[targetCode]);
  ...
  async function setCurrency(currency) {
    ...
    window.dispatchEvent(new CustomEvent("smarbiz-currency-change", { detail: { currency: key } }));
  }
